...
function PatientMyBookingsCard({ bookings = [], doctors = [] }) {
  ...
  // Keep myBookings in sync with bookings prop
  useEffect(() => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      {sortedBookings.length === 0 ? (
        <div style={{ textAlign: "center", marginTop: "1em", color: "#e0e0e0", fontWeight: 500 }}>
        ...
        sortedBookings.map((booking) => {
          ...
          const time = dateObj
            ...
            : "-";
          // Support both { _id, firstName, lastName } and { user: { _id }, firstName, lastName }
          let doctor = null;
          if (Array.isArray(doctors) && booking.doctorId) {
            doctor = doctors.find(
              (doc) =>
                (doc.user && doc.user._id === booking.doctorId) ||
                doc._id === booking.doctorId
            );
          }
